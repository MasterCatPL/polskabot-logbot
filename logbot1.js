...
function send() {
	try {
		...
		try {
			...
			let chans = client.channels.cache.filter(channel => channel.type === 'text').filter(channel => channel.name === 'polskahook-salmon').array();
...
client.on('ready', () => {
	client.user.setActivity('POLSKAHOOK gaming', { type: "PLAYING" });
